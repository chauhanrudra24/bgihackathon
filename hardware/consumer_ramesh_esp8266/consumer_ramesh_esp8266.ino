...
void ICACHE_RAM_ATTR flowPulseISR() { pulseCount++; }
...
void setEmergency(bool state, const char *source) {
  ...
  if (emergencyActive) {
    ...
    log.add("duration",
            millis() / 1000); // Simple uptime-based duration for context
    log.set("timestamp/.sv", "timestamp");
    Firebase.Firestore.createDocument(&fbdo, FIREBASE_PROJECT_ID, "",
                                      "emergencyLogs", log.raw());
  }

  Firebase.RTDB.setBool(&fbdo, F("sensorData/consumer_node/emergencyActive"),
                        emergencyActive);
  // For UI: expose emergencyValue as "SOS litres used" (since we removed quota)
  Firebase.RTDB.setFloat(&fbdo, F("sensorData/consumer_node/emergencyValue"),
                         emergencyLitres);
  Firebase.RTDB.setString(&fbdo, F("sensorData/consumer_node/emergencySource"),
                          source);
  digitalWrite(EMERGENCY_LED_PIN, emergencyActive ? HIGH : LOW);
  logAlert("Ramesh", "EMERGENCY",
           emergencyActive ? "Emergency ENABLED" : "Emergency DISABLED");
}
...
void setup() {
  ...
  if (Firebase.ready()) {
    if (Firebase.RTDB.getFloat(&fbdo,
                               F("sensorData/consumer_node/totalLitres"))) {
  ...
  attachInterrupt(digitalPinToInterrupt(FLOW_SENSOR_PIN), flowPulseISR,
                  FALLING);
  pinMode(EMERGENCY_BUTTON_PIN, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(EMERGENCY_BUTTON_PIN), buttonISR,
                  FALLING);
  lastFlowCalc = millis();
...
void loop() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    if (wifiDownStartTime == 0)
      wifiDownStartTime = millis();
    // Only attempt manual reconnect after 10s of sustained disconnect
    if (millis() - wifiDownStartTime > 10000 &&
  ...
  if (mpuInitialized) {
    ...
    if (!tamperDetected) {
      if (deltaX > TAMPER_THRESHOLD || deltaY > TAMPER_THRESHOLD ||
          deltaZ > TAMPER_THRESHOLD) {
        tamperDetected = true;
        Serial.println(F("!! INSTANT TAMPER DETECTED !!"));
        // Immediate Reporting
        Firebase.RTDB.setBoolAsync(
            &fbdo, F("sensorData/consumer_node/tamperDetected"), true);
        logAlert("Ramesh", "TAMPER",
                 "Instant physical interference detected! Supply blocked.");
      }
    }
  }
  ...
  if (Firebase.ready() && (millis() - lastControlCheckMs > 1000)) {
    ...
    if (Firebase.RTDB.getJSON(&fbdo, F("valves/consumer_node"))) {
      ...
      if (j.get(d, F("gov")) && d.success)
        gov = d.boolValue;
      if (j.get(d, F("user")) && d.success)
        usr = d.boolValue;
    ...
    if (Firebase.RTDB.getJSON(&fbdo, F("commands/consumer_node"))) {
      ...
      if (j.get(d, F("reset")) && d.success && d.boolValue) {
        ...
        Firebase.RTDB.setBool(
            &fbdo, F("sensorData/consumer_node/tamperDetected"), false);
        Firebase.RTDB.setBool(
            &fbdo, F("sensorData/consumer_node/emergencyActive"), false);
      }
      if (j.get(d, F("triggerEmergency")) && d.success && d.boolValue) {
        if (!emergencyActive)
          setEmergency(true, "WEB_DASHBOARD");
        Firebase.RTDB.setBool(
            &fbdo, F("commands/consumer_node/triggerEmergency"), false);
      }
      ...
      if (j.get(d, F("clearTamper")) && d.success && d.boolValue) {
        tamperDetected = false;
        lastTamperTime = 0;
        Firebase.RTDB.setBool(&fbdo, F("commands/consumer_node/clearTamper"),
                              false);
        Firebase.RTDB.setBool(
            &fbdo, F("sensorData/consumer_node/tamperDetected"), false);
        Firebase.RTDB.setBool(&fbdo, F("valves/consumer_node/gov"),
                              true); // UNBLOCK
  ...
  if (Firebase.ready() &&
      (millis() - lastSettingsSync > 15000 || lastSettingsSync == 0)) {
  ...
  if (Firebase.ready() && (millis() - lastRealtimeReport > 1000)) {
    ...
    Firebase.RTDB.setFloat(&fbdo, F("sensorData/consumer_node/flowRate"),
                           flowRate);
    Firebase.RTDB.setFloat(&fbdo, F("sensorData/consumer_node/totalLitres"),
                           totalLitres);
    Firebase.RTDB.setFloat(&fbdo, F("sensorData/consumer_node/emergencyLitres"),
                           emergencyLitres);
  ...
  if (Firebase.ready() &&
      (millis() - lastHeartbeat > 5000 || lastHeartbeat == 0)) {
    lastHeartbeat = millis();
    Firebase.RTDB.setBool(&fbdo, F("sensorData/consumer_node/valveState"),
                          currentValveState);
    Serial.printf(
        "Flow:%.2f | Bill:%.2f | Emg:%.2f | Tamper:%s | Valve:%s | Heap:%d\n",
        flowRate, totalLitres, emergencyLitres, tamperDetected ? "YES" : "No",
        currentValveState ? "OPEN" : "CLOSED", ESP.getFreeHeap());
  }
}
